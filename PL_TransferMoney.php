<?php
  session_start();
  require_once("../BusinessLayer/BL_TransferMoneyManager.php");
  if (isset($_POST["Send"])) {
    if(isset($_POST["Amount"]) && isset($_POST["IBAN"]) ) {
      $AccountId = $_SESSION['AccountId'];
      $Amount = trim($_POST["Amount"]);
      $IBAN = trim($_POST["IBAN"]);
      $Date = date("Y-m-d");
      $result = BL_TransferMoneyManager::InsertMoneyTransfer($AccountId,$IBAN,$Amount,$Date);
      if($result == 0) {
                        ...
                        <div class="col-md-4" style="margin-left: 300px;">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="text" style="width: 360px;" name="IBAN" placeholder="IBAN Number" required class="form-control"/>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="InputName">Amount</label>
                                <div class="input-group">
                                    <input type="text" style="width: 360px;" name="Amount" placeholder="Amount" required class="form-control"/>
                                </div>
                            </div>
                            <input type="submit" name="Send" value="Send" class="btn btn-info pull-left">
                        </div>
                     </form>
                 </div>
               </div>
